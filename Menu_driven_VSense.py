...
class ImageProcessingApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.image_labels = {}
        Image_Height = 320 # Assuming fixed height for image panels
        Image_Width =370
        ...
        #self.timer_recorded.timeout.connect(self.update_GreenScreen_video)
        ...
        for i in range(1, 4):  # Create three image panels
            ...
            image.setFixedSize(Image_Width, Image_Height)  # Assuming fixed height of 300
            self.image_labels[f"image_{i}"] = image

            shadow = QGraphicsDropShadowEffect()
            shadow.setOffset(10, 4)  # Shadow offset
            shadow.setBlurRadius(5)  # Shadow blur radius
            shadow.setColor(QColor(0, 0, 0, 250))  # Shadow color (RGBA)
    ...
    def create_menu(self):
        ...
        video_actions = {
            ...
            #"GreenScrenVideo":self.Play_GreenScreen_video
        }
        ...
        real_actions = {
            ...
            "Signature_matching":self.signature_matching,
            "GreenScrenVideo": self.Play_GreenScreen_video
    ...
    def upload_image(self, label):
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Image", "", "Images (*.png *.jpg *.jpeg *.bmp *mp4)")
        if file_path:
            ...
            if label == self.image_labels["image_1"]:
                ...
                _, ext = os.path.splitext(self.file_path1)
                if self.file_path1 and ext != ".mp4":
                    frame = cv2.imread(self.file_path)
                    self.upload_input_imgaes(frame, label)
                if ext == ".mp4":
                    input_video = cv2.VideoCapture (self.file_path1) #("C:/Users/Samsan/Downloads/GreenVideo.mp4")
                    ret_fg, frame = input_video.read()
                    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                    h, w, ch = frame.shape
                    bytes_per_line = ch * w
                    q_image = QImage(frame.data, w, h, bytes_per_line, QImage.Format_RGB888)
                    pixmap = QPixmap.fromImage(q_image)
                    #self.image_labels["image_1"].setPixmap(pixmap)
                    self.image_labels["image_1"].setPixmap(
                        pixmap.scaled(self.image_labels["image_1"].width(), self.image_labels["image_1"].height(),
                        Qt.KeepAspectRatio))

                    #while True:
                        # ret_fg, frame = input_video.read()
                        # frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                        # self.display_image(self.image_label1, frame)
                        # #cv2.imshow('I1', frame)
                        # cv2.waitKey(0)
                ...
                if self.file_path2 and ext != ".mp4":
                    frame = cv2.imread(self.file_path2)
                    self.upload_input_imgaes(frame, label)
                if ext == ".mp4":
                    input_video2 = cv2.VideoCapture(self.file_path2) #("C:/Users/Samsan/Downloads/GreenVideo.mp4")
                    ret_fg, frame = input_video2.read()
                    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                    #self.upload_input_imgaes(frame, label)
                    h, w, ch = frame.shape
                    bytes_per_line = ch * w
                    ...
                    self.image_labels["image_2"].setPixmap(
                        pixmap.scaled(self.image_labels["image_2"].width(), self.image_labels["image_2"].height(),
                                      Qt.KeepAspectRatio))
            try:
                if label == self.image_labels["image_3"]:
                    self.file_path3 = file_path
    ...
    def upload_input_imgaes(self, frame, label):
        #frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        h, w, ch = frame.shape
        bytes_per_line = ch * w
        q_image = QImage(frame.data, w, h, bytes_per_line, QImage.Format_RGB888)
        ...
        if label == self.image_labels["image_1"]:
            ...
            self.image_labels["image_2"].setPixmap(pixmap)
        else:
    ...
    def signature_matching(self):
         reference_signature = self.file_path1
         test_signature = self.file_path2

         refimage=self.image1
         testimage =self.image2

         reference_signature_image = cv2.cvtColor(refimage, cv2.COLOR_BGR2GRAY)
         test_signature_image = cv2.cvtColor(testimage, cv2.COLOR_BGR2GRAY)

         _, ref_image = cv2.threshold(reference_signature_image, 128, 255, cv2.THRESH_BINARY)
         _, test_image = cv2.threshold(test_signature_image, 128, 255, cv2.THRESH_BINARY)


         #ref_image = self.preprocess_image(self.file_path1)
         #test_image = self.preprocess_image(self.file_path2)

         # Resize to match dimensions
         test_image = cv2.resize(test_image, (ref_image.shape[1], ref_image.shape[0]))

         similarity_index = ssim(ref_image, test_image)
        ...
        #######
         ...
         kp1, des1 = orb.detectAndCompute(reference_signature_image, None)
         kp2, des2 = orb.detectAndCompute(test_signature_image, None)
         #
         ...
         matching_score = (len(matches) / max(len(kp1), len(kp2))) * 100
         #print(matching_score)
         #
         # print(f"Signature Matching Score: {matching_score:.2f}%")
         #
         # finalScore=(matching_score+similarity_index)/2
         cv2.putText(self.image1, f"Score: {int(similarity_index *100)}%", (20, 60),
    ...
    def Play_GreenScreen_video(self):
        input_video = cv2.VideoCapture (self.file_path1) #("C:/Users/Samsan/Downloads/GreenVideo.mp4")
        # Open background video
        background_video = cv2.VideoCapture (self.file_path2) #("C:/Users/Samsan/Downloads/GreenBackgroundVideo.mp4")
        ...
        while True:
            n=n+1
            ret_fg, foreground = input_video.read()
            ...
            # Convert foreground to HSV color space
            ...
            fg_clean = cv2.bitwise_and(foreground, foreground, mask=mask_inv)
            bg_clean = cv2.bitwise_and(background, background, mask=mask)

            final_frame = cv2.add(fg_clean, bg_clean)
    ...
    def run_crowd_count(self):
        if self.image1 is not None:
            ...
            #cv2.putText(self.image1, f"Count: {count}", (100, 60),
                        #cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 0), 4)

            # self.count_label = QLabel(self)
            # self.count_label.setText("Count: 35")  # Example count
            # self.count_label.setFont(QFont("Arial", 16, weight=QFont.Bold))
            # self.count_label.setStyleSheet("color: red; background-color: transparent;")
            # self.count_label.adjustSize()

            self.count_label = QLabel(self)
            self.count_label.setGeometry(950, 230, 150, 50)  # (x, y, width, height)

            self.count_label.setText("Count:" +str(count))  # Example count
            self.count_label.setFont(QFont("Arial", 16, weight=QFont.Bold))
            self.count_label.setStyleSheet("color: red; background-color: transparent;")
            self.count_label.adjustSize()
            self.count_label.show()

            # Position it above the image QLabel
            #self.count_label.move(self.image3.x() + 200, self.image3.y() - 30)
            self.image3 = self.image1
            self.display_image(self, self.image3)
    ...
    def update_frame(self):
       #print( self.current_mode)
        ...
        if self.current_mode == "greenScreenofflinevideo":
           self.update_Green_Screen_offline_video()
    ...
    def Play_GreenScreen_video1(self):
            # Start the QTimer to call `update_laneDeparture_video` every 30ms
            if self.video1 and self.video2:
                self.timer_recorded.start(30)

    def update_GreenScreen_video(self):
        ...
        fg_clean = cv2.bitwise_and(frame1, frame1, mask=mask_inv)
        bg_clean = cv2.bitwise_and(frame2, frame2, mask=mask)
        processed_frame = cv2.add(fg_clean, bg_clean)

        # Convert frames for QLabel display
        self.set_label_image(self.video_label1, frame1)
        self.set_label_image(self.video_label2, frame2)
        self.set_label_image(self.image3, processed_frame)

    def set_label_image(self, label, frame):
        """ Convert OpenCV frame to QPixmap and display in QLabel """
        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        ...
        for c in range(1, ROTATION_COUNT + 1):
           # print(f"Processing {c}")  # Equivalent to `app.ProcessButton.Text = num2str(c)`
            ...
            if len(idx) > 0:
                ...
                #IRot_original=cv2.resize(IRot_original, (500,500))
